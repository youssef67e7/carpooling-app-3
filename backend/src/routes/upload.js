...
router.post("/", authRequired, blockCheck, async (req, res) => {
  ...
  try {
    ...
    if (typeof image !== "string" || !image.startsWith("data:")) {
      return res
        .status(400)
        .json({ success: false, error: { code: "UPLOAD_ERROR", message: "Image must be a data URL (base64)" } });
    }
    ...
    const uploadResponse = await Promise.race([
      cloudinary.uploader.upload(image, {
        ...
        public_id: `${req.userId || "anon"}_${Date.now()}`,
...
router.post("/profile-image", authRequired, validate(saveProfileImageSchema), async (req, res) => {
  try {
    ...
    if (!imageUrl || !imageUrl.startsWith("https://res.cloudinary.com")) {
      return res
        .status(400)
        .json({ success: false, error: { code: "UPLOAD_ERROR", message: "A valid Cloudinary URL is required" } });
    }
