...
import { verificationProgress } from "../services/driverVerification.js";
...
router.post(
  ...
  body("cars.*.carCategory").optional({ values: "falsy" }).isIn(["sedan", "suv", "van", "electric"]),
  body("cars.*.registrationDocUrl").optional({ values: "falsy" }).isString().trim().isLength({ min: 4, max: 1000 }),
  body("cars.*.insuranceDocUrl").optional({ values: "falsy" }).isString().trim().isLength({ min: 4, max: 1000 }),
  body("cars.*.year").optional({ values: "falsy" }).toInt().isInt({ min: 1980, max: 2035 }),

  body("payoutBankName").optional({ values: "falsy" }).trim().isLength({ min: 2, max: 80 }),
  body("payoutAccountNumber").optional({ values: "falsy" }).trim().isLength({ min: 4, max: 64 }),
  body("payoutAccountHolder").optional({ values: "falsy" }).trim().isLength({ min: 2, max: 80 }),
  body("backgroundCheckConsent").optional().isBoolean(),
  ...
  async (req, res, next) => {
    try {
      ...
      for (const c of cars) {
        ...
        if (!seats) throw new AppError("Invalid seats", 400);
        c.seats = seats;
        c.carCategory = ["sedan", "suv", "van", "electric"].includes(String(c.carCategory || "").toLowerCase())
          ? String(c.carCategory).toLowerCase()
          : "sedan";
        if (c.year != null) c.year = Math.min(2035, Math.max(1980, Math.floor(Number(c.year) || 0)));
        if (c.registrationDocUrl) c.registrationDocUrl = String(c.registrationDocUrl).trim().slice(0, 500);
        if (c.insuranceDocUrl) c.insuranceDocUrl = String(c.insuranceDocUrl).trim().slice(0, 500);
      }
      ...
      if (req.body.payoutBankName) profSet.payoutBankName = String(req.body.payoutBankName).trim().slice(0, 80);
      if (req.body.payoutAccountNumber) profSet.payoutAccountNumber = String(req.body.payoutAccountNumber).trim().slice(0, 64);
      if (req.body.payoutAccountHolder) profSet.payoutAccountHolder = String(req.body.payoutAccountHolder).trim().slice(0, 80);
      if (req.body.backgroundCheckConsent != null) profSet.backgroundCheckConsent = !!req.body.backgroundCheckConsent;
      if (req.body.termsAccepted != null) profSet.termsAccepted = !!req.body.termsAccepted;
