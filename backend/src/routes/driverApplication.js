...
router.post(
  ...
  body("profileImageUrl").isString().trim().isLength({ min: 4, max: 500 }),

  body("criminalRecordFrontUrl").isString().trim().isLength({ min: 4, max: 500 }),
  body("criminalRecordBackUrl").isString().trim().isLength({ min: 4, max: 500 }),
  body("nationalIdNumber").custom((v) => isValidIdNumber(v)).withMessage("Invalid national id number"),

  body("licenseImageUrl").isString().trim().isLength({ min: 4, max: 500 }),
  ...
  body("carImageUrl").optional().isString().trim().isLength({ min: 4, max: 500 }),
  body("carBrand").optional().trim().isLength({ min: 1, max: 80 }),
  body("carModel").optional().trim().isLength({ min: 1, max: 80 }),
  body("carColor").optional().trim().isLength({ min: 1, max: 40 }),
  body("carPlateNumber").optional().trim().isLength({ min: 1, max: 24 }),
  body("numberOfSeats").optional().isInt({ min: 2, max: 20 }),
  body("cars").optional().isArray({ min: 1, max: 6 }),
  body("cars.*.imageUrl").optional().isString().trim().isLength({ min: 4, max: 500 }),
  body("cars.*.brand").optional().trim().isLength({ min: 1, max: 80 }),
  body("cars.*.model").optional().trim().isLength({ min: 1, max: 80 }),
  body("cars.*.color").optional().trim().isLength({ min: 1, max: 40 }),
  body("cars.*.plateNumber").optional().trim().isLength({ min: 1, max: 24 }),
  body("cars.*.seats").optional().isInt({ min: 2, max: 20 }),
  body("cars.*.carCategory").optional().isIn(["sedan", "suv", "van"]),
